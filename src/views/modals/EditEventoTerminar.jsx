...
export const EditEventoTerminar = ({ handleClose, setRefetch, evento }) => {
  ...
  const { values, handleChange, setValues } = useForm({
    ...
    numeroFormulario: evento.numeroFormulario,
    tipoEventoId: evento.tipoEvento.id,
    totalDias: evento.totalDias,
    totalHoras: evento.totalHoras,
    sectores: evento.sectores.map(s => s.id),
    niveles: evento.niveles.map(n => n.id),
    logros: evento.logros,
    compromisos: evento.compromisos,
    participantesHombres: evento.participantesHombres,
    participantesMujeres: evento.participantesMujeres,
    participantesComunitarios: evento.participantesComunitarios,
    participantesInstitucionales: evento.participantesInstitucionales,
    enlaceFormulario: evento.enlaceFormulario,
    enlaceFotografias: evento.enlaceFotografias,
    aprobar: false
  });
  ...
  const { setSend, send, data, isLoading, error } = useFetchPutBody(
    "eventos/finalizar",
    {
      ...values,
  ...
  return (
    <Card style={{ border: "none" }}>
      ...
      >
        ...
        <CloseButton onClick={() => handleClose({id: evento.id, success: true})} />
      </Card.Header>
      <Card.Body>
            ...
            <Card className="mb-4">
              ...
              <Card.Body className="p-4">
                {
                  aws ?
                  <InputFile label={'Formulario de participantes (un solo archivo jpg o pdf)'} idEvento={values.idEvento} prefix={'Formulario'} setValues={(url) => setValues({...values, enlaceFormulario: url})} edit={evento} enlace={evento?.enlaceFormulario}/>
                  :
                  <Form.Group as={Row} className="mb-3">
                    <Form.Label column sm="4" className="my-auto">
                      Enlace Formulario:
                ...
                }
                {
                  ...
                  <>
                    ...
                    <InputFile label={'Evidencias fotográficas (un solo archivo jpg, rar o pdf)'} idEvento={values.idEvento} prefix={'Fotografias'} setValues={(url) => setValues({...values, enlaceFotografias: url})} edit={evento} enlace={evento?.enlaceFotografias}/>
                  </>
                }
                </Card.Body>
            </Card>
