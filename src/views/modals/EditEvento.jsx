...
export const EditEvento = ({handleClose, setRefetch, evento, fixing=false}) => {
  ...
  const { values, handleChange, setValues } = useForm({
    ...
    idTarea: evento.idTarea || evento.tarea._id,
    nombre: evento.nombre,
    idAreaTematica: evento.idAreaTematica || evento.areaTematica._id,
    fechaInicio: evento.fechaInicio,
    fechaFinal: evento.fechaFinal,
    idDepartamento: evento.idDepartamento || evento.departamento._id,
    idMunicipio: evento.idMunicipio || evento.municipio._id,
    idAldea: evento.idAldea || evento.aldea._id,
    idCaserio: evento.idCaserio || evento.caserio._id,
    organizador: evento.organizador,
    componentes: evento.componentes.map(c => c._id),
  ...
  const { setSend, send, data, isLoading, error } = useFetchPutBody('eventos/crear', {...values,
  ...
  const handleSuccess = () => {
    handleClose()
    if(fixing){
      navigate('/reviews/eventos/crear/'+data._id)
      navigate(0)
    }
    else{
      setRefetch(true)
      setShowToast(true)
      actualizarTitulo('Evento Modificado')
  ...
  return (
    ...
    <Card.Header className="d-flex justify-content-between align-items-center" style={{backgroundColor: 'var(--main-green)', color: 'white'}}>
      <h4 className="my-1">Modificar Evento</h4>
