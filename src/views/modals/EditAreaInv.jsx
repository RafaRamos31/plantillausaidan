...
export const EditAreaInv = ({handleClose, setRefetch, area}) => {
  ...
  const handleNotFound = () => {
    handleClose()
    setShowToast(true)
    actualizarTitulo('Área Temática No Encontrada')
    setContent('El Área Temática que deseas modificar ya no existe.')
    setVariant('warning')
  }
  ...
  const { setSend: setSendEdit, send: sendEdit, data: dataEdit, isLoading: isLoadingEdit, error: errorEdit, code: codeEdit } = useFetchPutBody('areasinv', values) 
  ...
  const { setSend: setSendDelete, send: sendDelete, data: dataDelete, isLoading: isLoadingDelete, error: errorDelete, code: codeDelete } = useFetchDeleteBody('areasinv', {idArea: values.idArea}) 
  ...
  const handleSuccessDelete = () => {
    ...
    setRefetch()
    setShowToast(true)
    actualizarTitulo('Área Temática Eliminada')
    setContent('Área Temática eliminada correctamente.')
    setVariant('success')
  }
  ...
  return (
    ...
    <Card.Body>
      <Form onSubmit={handleSubmit}>
        <Form.Group as={Row} className="mb-3" controlId="idArea">
          <Form.Label column sm="4">
            uuid:
          </Form.Label>
          <Col sm="8">
            <Form.Control readOnly disabled value={values.idArea}/>
          </Col>
        </Form.Group>
