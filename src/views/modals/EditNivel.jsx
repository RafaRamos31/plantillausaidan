...
export const EditNivel = ({handleClose, setRefetchData, nivel, fixing=false}) => {
  ...
  const handleNotFound = () => {
    handleClose()
    setShowToast(true)
    actualizarTitulo('Nivel No Encontrado')
    setContent('El Nivel que deseas modificar ya no existe.')
    setVariant('warning')
  }
  ...
  const { setSend: setSendEdit, send: sendEdit, data: dataEdit, isLoading: isLoadingEdit, error: errorEdit, code: codeEdit } = useFetchPutBody('niveles', values) 
  ...
  const handleSuccessEdit = () => {
    handleClose()
    if(fixing){
      navigate('/reviews/niveles/'+dataEdit._id)
      navigate(0)
    }
    else{
      setRefetchData(true)
      setShowToast(true)
      actualizarTitulo('Nivel Modificado')
      setContent('Nivel guardado correctamente.')
      setVariant('success')
    }
  }
  ...
  return (
    ...
    <Card.Header className="d-flex justify-content-between align-items-center" style={{backgroundColor: 'var(--main-green)', color: 'white'}}>
      <h4 className="my-1">Modificar Nivel</h4>
