...
import { CrearNivel } from "./modals/CrearNivel.jsx";
import { EditNivel } from "./modals/EditNivel.jsx";

export const ConfigNiveles = () => {
        ...
        return (
          <>
            <OverlayTrigger overlay={<Tooltip>{'Ver'}</Tooltip>}>
              <a href={`/reviews/niveles/${params.row._id}`} target="_blank" rel="noreferrer">
            ...
            {
              user.userPermisos?.acciones['Niveles']['Modificar'] 
              &&
              <>
              {
                params.row.editing ?
  ...
  return(
    <>
    <Layout pagina={`Configuración - Niveles`} SiteNavBar={ConfigNavBar} breadcrumbs={[
        {link: '/', nombre: 'Inicio'},
        {link: '/configuracion', nombre: 'Configuración'},
        {link: '/configuracion/niveles', nombre: 'Niveles'}
    ]}>
      <div className="d-flex gap-2 align-items-center">
      <h2 className="view-title"><i className="bi bi-boxes"></i>{` Niveles`}</h2>
      {/*Boton Actualizar*/}
      {
          !updating ? 
      ...
      {/*Boton Agregar*/}
      {
        user.userPermisos?.acciones['Niveles']['Crear']
        &&
      ...
      {/*Boton Deleteds*/}
      {
        user.userPermisos?.acciones['Niveles']['Ver Eliminados'] 
        &&
        <>
        {
          !deleteds ?
    ...
    <Modal show={showEdit} onHide={handleCloseEdit} backdrop="static">
      <EditNivel handleClose={handleCloseEdit} setRefetchData={setRefetchData} nivel={currentData}/>
    </Modal>
    <Modal show={showCreate} onHide={handleCloseCreate} backdrop="static">
      <CrearNivel handleClose={handleCloseCreate} setRefetch={handleUpdate}/>
    </Modal>
    </>
  );
}
