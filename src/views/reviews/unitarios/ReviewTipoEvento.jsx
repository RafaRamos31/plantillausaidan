...
export const ReviewTipoEvento = () => {
  ...
  const { setSend, send, data, isLoading, error } = useFetchPutBody('revisiones/tiposEventos', values) 
  ...
  const handleSuccess = () => {
    ...
    setCharging(false)
    setShowToast(true)
    actualizarTitulo(`Tipo de Evento Revisado`)
    setContent(`Tipo de Evento revisado correctamente.`)
    setVariant('success')
  }
  ...
  return (
    ...
    <Layout pagina={`Revisión Tipo de Evento`} SiteNavBar={ConfigNavBar} breadcrumbs={[
      ...
      <Row className='mx-0 my-0'>
        <Col md={8}>
        <h2><i className="bi bi-postcard-fill"></i>{` Tipo de Evento`}</h2>
        ...
        <Col md={4}>
          <Form onSubmit={handleSubmit}>
            ...
            {
              user.userPermisos?.acciones['Tipos de Eventos']['Eliminar'] 
              &&
              <DeleteButton  charging={charging} dataRevision={dataRevision} type={`tiposEventos`} handleSubmit={handleSubmit}/>
            }
