...
export const ReviewSector = () => {
  ...
  const { setSend, send, data, isLoading, error } = useFetchPutBody('revisiones/sectores', values) 
  ...
  return (
    ...
    <Layout pagina={`Revisión ${endpoint.charAt(0).toUpperCase() + endpoint.slice(1)}`} SiteNavBar={ClientesNavBar} breadcrumbs={[
      ...
      <Row className='mx-0 my-0'>
        <Col md={8}>
        <h2><i className="bi bi-diagram-2-fill"></i>{` ${endpoint.charAt(0).toUpperCase() + endpoint.slice(1)}`}</h2>
          ...
          <Container style={{border: '1px solid lightgray', padding: '1.5rem', borderRadius: '10px', marginTop: '1rem'}}>
            <Row>
              <Col md={6}>
                ...
                <CompareValue  title={'Nombre del Sector:'} value={dataRevision.nombre} original={original?.nombre} compare={compare}/>
              </Col>
              {
                compare &&
                <Col md={6}>
                  ...
                  <CompareValue  title={'Nombre del Sector:'} value={dataRevision.nombre} original={original?.nombre} compare={compare} hidden/>
                </Col>
              }
            </Row>
        ...
        <Col md={4}>
          <Form onSubmit={handleSubmit}>
            ...
            {
              user.userPermisos?.acciones['Sectores']['Eliminar'] 
              &&
