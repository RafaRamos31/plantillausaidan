...
export const ReviewNivel = () => {
  const { idRevision } = useParams();

  const { user } = useContext(UserContext);

  //Peticio de datos a la API
  const { data: dataRevision, isLoading: isLoadingRevision, error: errorRevision, setRefetch } = useFetchGet(`niveles/id/${idRevision}`);
  ...
  useEffect(() => {
    if(dataRevision && dataRevision.originalId){
      setQueryOriginal(`niveles/id/${dataRevision.originalId}`)
  ...
  const { setSend, send, data, isLoading, error } = useFetchPutBody('revisiones/niveles', values) 
  ...
  const handleSuccess = () => {
    ...
    setCharging(false)
    setShowToast(true)
    actualizarTitulo(`Nivel Revisado`)
    setContent(`Nivel revisado correctamente.`)
    setVariant('success')
  }
  ...
  return (
    <>
    <Layout pagina={`Revisión Tipo de Evento`} SiteNavBar={ConfigNavBar} breadcrumbs={[
        {link: '/', nombre: 'Inicio'},
        {link: '/configuracion', nombre: 'Configuración'},
        {link: '/configuracion/niveles', nombre: 'Niveles'},
        {link: `/reviews/niveles/${idRevision}`, nombre: dataRevision?.nombre || 'Revisión'}
    ]}>
      <Row className='mx-0 my-0'>
        <Col md={8}>
        <h2><i className="bi bi-boxes"></i>{` Nivel`}</h2>
        ...
        <Col md={4}>
          <Form onSubmit={handleSubmit}>
            ...
            {
              user.userPermisos?.acciones['Niveles']['Eliminar'] 
              &&
              <DeleteButton  charging={charging} dataRevision={dataRevision} type={`niveles`} handleSubmit={handleSubmit}/>
            }
