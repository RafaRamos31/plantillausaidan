...
export const HistoryCargo = () => {
  ...
  return (
    <Layout pagina={`Historial - ${endpoint.charAt(0).toUpperCase() + endpoint.slice(1)}`} SiteNavBar={ClientesNavBar} breadcrumbs={[
      {link: '/', nombre: 'Inicio'},
      {link: '/clientes', nombre: 'Clientes'},
      {link: '/clientes/cargos', nombre: 'Cargos'},
      {link: `/historial/cargos/${id}`, nombre: `Historial: ${original?.nombre || 'Cargo'}`}
  ]}>
                ...
                <Accordion.Item id={rev._id} key={rev._id} eventKey={rev._id}>
                  ...
                  <Accordion.Body>
                    ...
                    <Container style={{border: '1px solid lightgray', padding: '1.5rem', borderRadius: '10px', marginTop: '0.2rem'}}>
                      <Row>
                        <Col md={6}>
                          <CompareValue  title={'Nombre del Cargo:'} value={rev.nombre} original={original?.nombre} compare={false}/>
                          <CompareValue  title={'Sector:'} value={rev.sector.nombre} original={rev?.sector.nombre} compare={false}/>
                        </Col>
                      </Row>
                    </Container>
                  </Accordion.Body>
                ...
                <Accordion.Item eventKey='info'>
                  ...
                  <Accordion.Body className='px-0 py-0'>
                    <Container  className='mx-0 my-0' style={{padding: '1.5rem'}}>
                      <CompareValue  title={'Nombre del Cargo:'} value={original.nombre} original={original?.nombre} compare={false}/>
