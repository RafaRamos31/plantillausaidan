...
export const HistoryQuarter = () => {
  ...
  return (
    <Layout pagina={`Historial - ${endpoint.charAt(0).toUpperCase() + endpoint.slice(1)}`} SiteNavBar={IndicadoresNavBar} breadcrumbs={[
      ...
      {link: `/historial/quarters/${id}`, nombre: `Historial: ${original?.nombre || 'Trimestre'}`}
  ]}>
      <Row className='mx-0 my-0'>
        <Col md={8}>
          <h2 className='mb-4'><i className="bi bi-clock-history"></i>{` Historial: Trimestre`}</h2>
          <Accordion>
            {
                ...
                <Accordion.Item id={rev._id} key={rev._id} eventKey={rev._id}>
                  ...
                  <Accordion.Body>
                    ...
                    <Container style={{border: '1px solid lightgray', padding: '1.5rem', borderRadius: '10px', marginTop: '0.2rem'}}>
                      <Row>
                        <Col md={6}>
                          <CompareValue  title={'Trimestre:'} value={rev.nombre} original={original?.nombre} compare={false}/>
                          <CompareValue  title={'Fecha de Inicio:'} value={moment(rev.fechaInicio).format('DD/MM/YYYY - HH:mm')} original={moment(original?.fechaInicio).format('DD/MM/YYYY - HH:mm')} compare={false}/>
                          <CompareValue  title={'Fecha de Finalización:'} value={moment(rev.fechaFinal).format('DD/MM/YYYY - HH:mm')} original={moment(original?.fechaFinal).format('DD/MM/YYYY - HH:mm')} compare={false}/>
                        </Col>
                      </Row>
                    </Container>
                  </Accordion.Body>
                ...
                <Accordion.Item eventKey='info'>
                  ...
                  <Accordion.Body className='px-0 py-0'>
                    <Container  className='mx-0 my-0' style={{padding: '1.5rem'}}>
                      <CompareValue  title={'Trimestre:'} value={original.nombre} original={original?.nombre} compare={false}/>
                      <CompareValue  title={'Fecha de Inicio:'} value={moment(original?.fechaInicio).format('DD/MM/YYYY - HH:mm')} original={moment(original?.fechaInicio).format('DD/MM/YYYY - HH:mm')} compare={false}/>
                      <CompareValue  title={'Fecha de Finalización:'} value={moment(original?.fechaFinal).format('DD/MM/YYYY - HH:mm')} original={moment(original?.fechaFinal).format('DD/MM/YYYY - HH:mm')} compare={false}/>
                    </Container>
                  </Accordion.Body>
                </Accordion.Item>
              </Accordion>
