...
export const ConfigAldeas = () => {
  ...
  const columns = [
    ...
    { field: 'geocode', headerName: 'Geocode', width: 120, description: 'Codigo Unico de la Aldea.', 
      filterOperators: getGridStringOperators().filter(
        (operator) => operator.value === 'contains',
      )},
      ...
      renderCell: (params) => {
        return (
          <>
            ...
            {
              user.userPermisos?.acciones['Aldeas']['Modificar'] 
              &&
              <>
              {
                params.row.editing ?
  ...
  return(
    ...
    <Layout pagina={`Configuración - ${endpoint.charAt(0).toUpperCase() + endpoint.slice(1)}s`} SiteNavBar={ConfigNavBar} breadcrumbs={[
      ...
      {/*Boton Deleteds*/}
      {
        user.userPermisos?.acciones['Aldeas']['Ver Eliminados'] 
        &&
        <>
        {
          !deleteds ?
