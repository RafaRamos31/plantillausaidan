...
export const ClientCargos = () => {
  ...
  const columns = [
    ...
    { field: 'nombre', headerName: 'Nombre del Cargo', width: 330,
      filterOperators: getGridStringOperators().filter(
    ...
    { field: 'sector', headerName: 'uuid Sector', width: 250},
      ...
      renderCell: (params) => {
        return (
          <>
            ...
            {
              user.userPermisos?.acciones['Cargos']['Modificar'] 
              &&
              <>
              {
                params.row.editing ?
  ...
  return(
    <>
    <Layout pagina={`Clientes - ${endpoint.charAt(0).toUpperCase() + endpoint.slice(1)}s`} SiteNavBar={ClientesNavBar} breadcrumbs={[
        {link: '/', nombre: 'Inicio'},
        {link: '/clientes', nombre: 'Clientes'},
        {link: '/clientes/cargos', nombre: 'Cargos'}
    ]}>
      <div className="d-flex gap-2 align-items-center">
        <h2 className="view-title"><i className="bi bi-person-badge"></i>{`${endpoint.charAt(0).toUpperCase() + endpoint.slice(1)}s`}</h2>
        {/*Boton Actualizar*/}
        {
          !updating ? 
