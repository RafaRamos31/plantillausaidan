...
import { CrearResultado } from "./modals/CrearResultado.jsx";
import { EditResultado } from "./modals/EditResultado.jsx";

export const PlanResultados = () => {
  ...
  return(
    ...
    <Layout pagina={`Planificación - ${endpoint.charAt(0).toUpperCase() + endpoint.slice(1)}s`} SiteNavBar={PlanNavBar} breadcrumbs={[
      ...
      <h2 className="view-title"><i className="bi bi-bar-chart-fill"></i>{` ${endpoint.charAt(0).toUpperCase() + endpoint.slice(1)}s`}</h2>
      {/*Boton Actualizar*/}
      {
          !updating ? 
      ...
      {/*Boton Agregar*/}
      {
        user.userPermisos?.acciones['Resultados']['Crear']
        &&
      ...
      {/*Boton Deleteds*/}
      {
        user.userPermisos?.acciones['Resultados']['Ver Eliminados'] 
        &&
        <>
        {
          !deleteds ?
    ...
    <Modal show={showCreate} onHide={handleCloseCreate} backdrop="static">
      <CrearResultado handleClose={handleCloseCreate} setRefetch={handleUpdate}/>
    </Modal>
    </>
  );
}
